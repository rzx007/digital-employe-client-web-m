...
todos:
  ...
  - id: settings-read
    ...
    status: completed
  - id: ui-list
    content: models-settings 主列表：已接入供应商+模型清单；Radio 单选 active（禁止 Switch）；编辑/删除/测试
    status: pending
  - id: ui-add
...
- 展示**已接入**的供应商及其模型列表（可多家并存，各自 Key 独立保存）
- 可从 [`catalog.py`](apps/server/src/llm/providers/catalog.py) 预设新增，或自定义供应商（id / 显示名 / base_url / api_key / 模型列表）
- 各供应商凭证**独立持久化**
- 用户**手动**在列表中指定「当前使用」的**唯一** `(provider_id, model_id)`（全局单选，供 Agent 调用）
- 第一期**不做**自定义请求头（Phase 2）

**与参考截图（Cursor「管理模型」）的差异**（勿照搬 Toggle）：

| 概念 | Cursor 截图 | 本应用 |
|------|-------------|--------|
| 分组标题 | 各 Plan / 供应商 | 已接入供应商 `display_name` |
| 模型行右侧控件 | Switch，语义像「启用/出现在选择器」 | **单选**（Radio 或「设为当前」按钮），全局仅一项为 active |
| 可同时 ON 的模型数 | 截图允许多个 Switch 独立开关 | **运行时只允许 1 个** `active_provider_id` + `active_model_id` |
| `providers[]` 里的 `models[]` | — | 该供应商**可用模型清单**（库存），不是 per-model 启用位 |

已接入 ≠ 正在使用：DashScope、DeepSeek 可同时「接入」并保留 Key，但 Agent 对话只读 **active** 那一套（镜像四键）。
...
- **models**：至少 1 个；`id` 为 API model 名，`display_name` 可选（UI 展示用）；**无** `enabled` 字段——列表仅描述该供应商下可切换的模型名，不表示多模型并行启用
- **active**：全局**至多一对** `(active_provider_id, active_model_id)`；可为 null；用户点「设为当前使用」后写入，并**自动取消**上一 active（互斥，等同 RadioGroup）
...
| PUT | `/model/active` | `{ provider_id, model_id }` 设为**唯一** active（覆盖旧 active）+ 镜像四键 |
...
### 4.1 主列表页（单选，不用 Switch）

布局可参考截图的分组列表，但交互必须服从**全局单模型**：

- **已接入供应商**（Card / Accordion）：显示名称、base_url（截断）、其下 `models[]` 清单
- 每个模型行：
  - 使用 **Radio**（推荐）或带互斥逻辑的「设为当前使用」按钮
  - **禁止** 使用 Switch / Checkbox 表达「启用」——避免用户以为可同时开多个模型
  - 当前 active 行：勾选图标 / 主色描边 / `当前使用` Badge（全列表仅一处）
- 点击另一模型 → `PUT /model/active` → 更新 `active_*` 并镜像四键；其它行自动变为未选中
- 供应商级操作：编辑、删除、测试连接（测的是该供应商凭证，不改变 active，除非用户随后点选）
- 顶部固定条：`当前使用：DashScope · deepseek-v4-flash`（无 active 时显示「未选择」）
- **添加供应商** 按钮（对应截图「+ 连接提供商」）
...
- 按员工/会话分模型
- 运行时自动 failover / **多 active**（与 Cursor 式多 Switch 启用无关）
- 模型级 `enabled` 开关、自定义模型选择器里「勾选多个模型同时可用」
