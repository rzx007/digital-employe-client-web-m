...
def build_leader_brief(
    ...
    return (
        ...
        "直接按用户答案拆解派活。\n"
        f"{plan_step}"
        "- **优先一次性把所有子任务放进同一次 `create_orchestration_plan` 调用**；"
        "若分步 create，系统会自动把后续任务追加到同一个计划（无需担心重复）。"
        "**严禁**因为想增减或调整任务就 cancel 已建的正确计划——增减任务直接再 create"
        "（会自动合并），改现有任务用 `update_task`，绝不要推倒重建。\n"
        "成员产出会自动汇总到群里。\n\n"
        f"用户需求：{question}"
    )
