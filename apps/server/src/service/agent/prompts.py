...
{file_tool_rules}
        ...
        - **长命令与后台（像 Cursor 一样有节奏）**：一般命令（查目录/取数/git 等几秒完成）直接 shell_execute、不传 timeout、同步拿结果。预判是长任务（拉镜像/全盘扫描/大型编译/下载）时，设 `run_in_background=True` 直接后台、或传较大 `timeout` 超时自动转后台——两者都立即返回 `session_id`，**输出不丢失**，同时显示在用户的「后台命令」面板里。
        - 拿到 `session_id` 后**别干等也别狂查**：可在**同一轮**先去做别的（写脚本/准备下一步），需要结果时用 `shell_wait(session_id, N)`（N 如 30-60s）有节奏地等一轮，没完成再等一轮；只想瞄一眼用 `shell_poll`。绝大多数任务等一两轮就完成。
        - 只有等了几轮、判断是**真·超大任务**（远未完、预估还很久）时，才告诉用户「这个任务较耗时、已在后台运行，你可以在后台命令面板查看进度，或稍后问我」并体面收尾——**绝不**因为「还没完成」就 `shell_kill` 杀掉重试或换命令重来。命令没报错就是在正常跑，耐心等。
...
def build_memory_update_section() -> str:
    ...
    return """
        ...
        ### 主动记住「稳定环境事实」（无需用户开口）
        - 排查/执行中发现**会复用、跨任务稳定**的环境或工具链事实时，**当场**调一次
          `remember_memory(section="已知事实与约定")` 记下，免得下次重新踩坑。
        - 典型该记：
          - 运行环境/OS、关键工具的可用路径。例：发现本机有两个 Python（默认 `.venv` 的 python、
            pip 装在系统 Python311），跑需系统包（如 networkx）的脚本要用 Python311 完整路径
            → `remember_memory(text="本机两个 Python：默认 .venv 的 python；pip/系统包(networkx 等)在系统 Python311，跑这类脚本用 Python311 完整路径", section="已知事实与约定")`
          - 某命令/调用方式的固定坑（如某工具需加 `-u`、某依赖的安装位置）
        - **判据**：只记**稳定、可复用、下次仍成立**的事实；**不记**任务专属/一次性/会变的内容
          （具体文件名、本次数据、临时状态）。记前对照已加载的记忆，已有就别重复记；一条一调用、简洁。
        """
