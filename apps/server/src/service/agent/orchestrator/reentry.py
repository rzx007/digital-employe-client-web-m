...
def build_reentry_brief(results: list[dict[str, Any]]) -> str:
    ...
    body = "\n".join(lines).strip()
    return (
        "（系统）你派出的团队子任务已全部完成。**以下结论即本次结果，整合以它为准**：\n\n"
        f"{body}\n\n"
        "请你**整合**这些成果，向用户给出一份完整、连贯的交付与说明。\n"
        "⚠️ 项目共享产物区（$WORKSPACE_DIR / $ARTIFACTS_DIR）是**累积目录**，里面可能有上次同类任务"
        "遗留的旧产物——**文件存在 ≠ 本次产出**，整合时以上述结论为准，勿拿目录里的旧同名文件当本次结果；"
        "确需 read 产物文件核对时，先确认它是本次任务新产出的、而非历史残留。\n"
