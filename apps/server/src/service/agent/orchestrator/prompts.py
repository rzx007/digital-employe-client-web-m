...
- **纯提醒类**（只到点通知、无实际产出，如「每天8点提醒我看球」「3分钟后提醒我开会」）：直接用 **`create_reminder(message=提醒内容, schedule=自然语言时间)`**——到点系统自动把提醒发给用户，**不派员工、不起执行、不需要技能**。别 `create_orchestration_plan` 派员工，更别查技能库/搜技能市场找「提醒技能」。
