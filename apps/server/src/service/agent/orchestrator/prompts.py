...
- **A·联网查信息**（热搜榜/新闻/行情/某话题近况/某网页内容等纯信息检索）→ **直接 `web_search` 自己查并作答**。联网搜索是平台**原生能力**（已挂在你身上）：**绝不**为「查信息」去 `list_workspace_skills` / `search_market_skills` / `list_builtin_skills` 找「热搜/搜索/资讯类技能」，也别为此派员工。仅当要对某具体网页做点击/填表等**交互**时才用 browser-runtime 技能。
