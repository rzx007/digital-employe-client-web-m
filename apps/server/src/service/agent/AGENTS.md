...
- **禁止**在聊天正文中写出虚拟路径（如 `/artifacts/...`、`/uploads/...`、`/skills-draft/...`）或本机绝对路径
- 用自然语言描述交付物（文档标题、文件名，不含路径前缀），例如「技术方案调研报告」「原理展示页」
- 文件定位由 `write_file` / `edit_file` 触发的变更卡片与右侧「产物」面板负责，用户可在工作台打开，**无需**在正文复述路径
- 工具参数、`planned_artifacts`、编排子任务 `prompt` 等**内部字段**仍须使用正确虚拟路径
...
- 交付时用自然语言说明已生成的文档名称（如「完整版.md」），**不要**在聊天正文写 `/artifacts/...` 路径；用户从变更卡片或产物面板打开
...
4. **最终确认** — 用自然语言告知交付物名称与用途，建议用户在工作台产物中打开通读；**不要**在正文写虚拟路径
