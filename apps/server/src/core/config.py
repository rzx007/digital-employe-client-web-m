...
def get_settings() -> Settings:
    ...
    legacy_employee_zip_url = _get_env_value("EMPLOYEE_ZIP_URL")
    legacy_skill_remote_base_url = _get_env_value("SKILL_REMOTE_BASE_URL")
    legacy_login_url = _get_env_value("LOGIN_URL")
    legacy_mcp_base_url = _get_env_value("MCP_BASE_URL")
    ...
    remote_api_base_url = (
        _get_settings_value(settings_data, "endpoint")
        or _get_env_value("REMOTE_API_BASE_URL")
        or legacy_skill_remote_base_url
        or employee_zip_legacy_base
    )
    employee_zip_path = (
        _get_env_value("EMPLOYEE_ZIP_PATH")
        or employee_zip_legacy_path
        or "/api/v1/employees/export"
    )
    skill_remote_list_path = _get_env_value("SKILL_REMOTE_LIST_PATH") or _get_env_value(
        "SKILL_REMOTE_LIST_URL"
    ) or "/api/v1/client/skills/export"
    skill_remote_detail_path = _get_env_value(
        "SKILL_REMOTE_DETAIL_PATH"
    ) or "/api/v1/client/skills/export/full/{skill_id}"

    mcp_remote_list_url = _get_env_value("MCP_REMOTE_LIST_PATH") or _get_env_value(
        "MCP_REMOTE_LIST_URL"
    )
    mcp_remote_detail_url = _get_env_value("MCP_REMOTE_DETAIL_PATH") or _get_env_value(
        "MCP_REMOTE_DETAIL_URL"
    )

    platform_base_url = (
        _get_env_value("PLATFORM_BASE_URL")
        or login_legacy_base
        or mcp_legacy_base
    )
    mcp_client_base_path = (
        _get_env_value("MCP_CLIENT_BASE_PATH")
        or mcp_legacy_path
        or "/llm/aios/mcp/client"
    )
    login_path = _get_env_value("LOGIN_PATH") or login_legacy_path or "/yc/login"
    ...
    return Settings(
        ...
        mcp_tool_call_path=_get_env_value("MCP_TOOL_CALL_PATH") or "/tool/call",
        ...
        agent_interface_skill_prefix=_get_env_value("AGENT_INTERFACE_SKILL_PREFIX")
        or "/aios/skill",
        dbchat_base_url=_get_env_value("DBCHAT_BASE_URL"),
        dbchat_model_chat_simple_path=_get_env_value(
            "DBCHAT_MODEL_CHAT_SIMPLE_PATH"
