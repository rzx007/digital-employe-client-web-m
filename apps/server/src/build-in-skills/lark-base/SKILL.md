...
> **前置条件：** 先阅读 `[./references/lark-shared.md](./references/lark-shared.md)`；**全新机器**须先按其中「环境准备 / 首次安装」完成 Node/npm 与全局 `@larksuite/cli`，再执行 `config init` / `auth login` 等。
> **执行前必做：** 执行任何 `base` 命令前，必须先阅读对应命令的 reference 文档，再调用命令。
> **命名约定：** 仅使用 `lark-cli base +...` 形式的命令。
> **基础的base-token** 研发管理的多维表格的 base-token为 UrEbbbWQOan8RtsZxvIcQZBhn9b
