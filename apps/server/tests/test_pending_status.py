...
def _seed_plan_AB(db, *, conv_id=555):
    ...
    # 开一轮 PlanRun，供 latest_run_id_for_plan / _load_accepted_task_ids 使用
    run = PlanRun(plan_id=plan.id, workspace_id=ws.id, run_seq=1,
                  trigger="manual", auto_accept=False, status="running")
    db.add(run); db.commit()
    return ws, emp, plan, A, B, run


def _accept_log(db, *, task, ws_id, emp_id, run_id):
    log = TaskExecutionLog(
        ...
        reported_at=cst_now(), qa_accepted_at=cst_now(), run_id=run_id,
    )
...
def test_waiting_status_pending_release(db_session):
    ...
    ws, emp, plan, A, B, run = _seed_plan_AB(db_session)
    _accept_log(db_session, task=A, ws_id=ws.id, emp_id=emp.id, run_id=run.id)
...
def test_snapshot_shows_pending_release_section(db_session):
    ...
    ws, emp, plan, A, B, run = _seed_plan_AB(db_session)
    _accept_log(db_session, task=A, ws_id=ws.id, emp_id=emp.id, run_id=run.id)  # A: log+accepted; B: no log
