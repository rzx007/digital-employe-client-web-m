...
>
> **文件不在产物目录时**（如 skill 自己的 `output/` 目录）：`open-artifact` 只能打开产物目录（`/artifacts`、`/uploads`、`/skills-draft`）内的文件，否则报 `PATH_NOT_IN_ARTIFACTS`。先把 HTML 复制进产物目录再打开（shell 的 cwd 就是产物 `artifacts` 目录）：
> ```bash
> cp "<skill 输出的物理路径>" ./report.html
> browserctl open-artifact /artifacts/report.html
> ```
