...
- 用户已在内嵌浏览器登录公司 OA
- 演示 `browser_*` 多步填表流程（实际 URL/选择器需按贵司 OA 调整）

## LLM 行为提示

1. 从用户提问提取 `start_time`、`end_time`、`reason`
2. `browser_navigate` 到加班申请页（或用户当前 OA 地址）
3. 按 frontmatter 选择器依次 `browser_fill` / `browser_click`
4. 提交前对 `#submit` 使用 `browser_click("#submit", confirmation_required=True, confirmation_message="确认提交加班申请？")`；用户取消则停止

## 注意

- `oa.example.com` 为占位域名；离线或无法访问时请说明并改用手动指引
- 选择器与真实 OA 不一致时，用 `browser_snapshot` 重新定位
