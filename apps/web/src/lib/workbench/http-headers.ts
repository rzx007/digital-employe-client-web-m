 ...
 */
export function normalizeHeadersFromUnknown(raw: unknown): Record<string, string> | undefined {
...
function objectToHeaderRecord(raw: unknown): Record<string, string> | undefined {
...
function parseLooseJsStyleHeaderObject(slice: string): Record<string, string> | undefined {
  const out: Record<string, string> = {}
 ...
 */
function parseBalancedJsonObject(text: string, openBraceIndex: number): Record<string, string> | undefined {
...
function mergeHeaderJsonObjects(text: string, into: Record<string, string>): void {
...
/** `请求头：{ "Authorization": "..." }` */
function extractChineseHeadersJson(text: string, into: Record<string, string>): void {
  const re = /请求头\s*[：:]\s*\{/g
  let m: RegExpExecArray | null
  while ((m = re.exec(text)) !== null) {
    const braceStart = m.index + m[0].length - 1
...
function extractFromCurlHeaders(text: string, into: Record<string, string>): void {
...
function extractFromMarkdownHeaderTables(text: string, into: Record<string, string>): void {
  const headerCols =
  ...
  for (const m of text.matchAll(headerCols)) {
    ...
    let value = m[2].trim()
    if (!value) continue
    if (name.toLowerCase() === "api-key" || name === "X-API-KEY") into["X-Api-Key"] = stripQuotes(value)
 ...
 */
function extractRequestHeadersSection(text: string, into: Record<string, string>): void {
  const blockRe = /请求头\s*[：:]?\s*\n([\s\S]*?)(?=\n\s*(?:响应示例|响应\s*[：:]|#\s|$))/i
 ...
 */
function extractFromHeaderLines(text: string, into: Record<string, string>): void {
  const lineRe =
 ...
 */
export function extractHeadersFromSkillText(text: string): Record<string, string> {
  const out: Record<string, string> = {}
