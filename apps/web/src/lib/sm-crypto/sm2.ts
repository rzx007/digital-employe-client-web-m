...
export default class SM2Cipher {
  ct: number
  p2: ECPointFp | null
  sm3keybase: SM3Digest | null
  sm3c3: SM3Digest | null
  key: number[]
  keyOff: number
  ...
  initEncipher(userKey: ECPointFp): string {
  ...
  initDecipher(userD: BigInteger, c1: ECPointFp): void {
    this.p2 = c1.multiply(userD)
    this.reset()
  }

  decryptBlock(data: number[]): void {
    for (let i = 0; i < data.length; i++) {
      if (this.keyOff === this.key.length) {
        this.nextKey()
      }
