...
export default class SM3Digest {
  ...
  processWord(in_Renamed: number[], inOff: number): void {
  ...
  processLength(bitLength: number): void {
    if (this.xOff > 14) {
      this.processBlock()
    }
  ...
  intToBigEndian(n: number, bs: number[], off: number): void {
  ...
  doFinal(out_Renamed: number[], outOff: number): number {
    this.finish()
    for (let i = 0; i < 8; i++) {
  ...
  blockUpdate(input: number[], inOff: number, length: number): void {
  ...
  rotate(x: number, n: number): number {
  ...
  ff_00_15(X: number, Y: number, Z: number): number {
    return X ^ Y ^ Z
  }

  ff_16_63(X: number, Y: number, Z: number): number {
    return (X & Y) | (X & Z) | (Y & Z)
  }

  gg_00_15(X: number, Y: number, Z: number): number {
    return X ^ Y ^ Z
  }

  gg_16_63(X: number, Y: number, Z: number): number {
    return (X & Y) | (~X & Z)
  }

  urShift(number: number, bits: number): number {
  ...
  urShiftLong(number: number, bits: number): number {
    let returnV
  ...
  getZ(g: ECPointFp, publicKey: string, userId?: string): number[] {
