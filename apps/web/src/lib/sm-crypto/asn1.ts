 ...
 */
function getStartPosOfV(s: string, pos: number): number {
 ...
 */
function getHexOfV(s: string, pos: number): string {
 ...
 */
function getPosOfNextSibling(s: string, pos: number): number {
  const pos1 = getStartPosOfV(s, pos)
  const len = getIntOfL(s, pos)
  return pos1 + len * 2
}
 ...
 */
function getPosArrayOfChildren(h: string, pos: number): number[] {
  const a: number[] = []
 ...
 * 解析 ASN.1 DER
 */
export function decodeDer(input: string): { r: BigInteger; s: BigInteger } {
