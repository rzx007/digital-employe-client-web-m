  ...
  const renderEmployeeRow = (employee: Employee) => {
    ...
    return (
      <Fragment key={employee.id}>
        <div
          className={cn(
            "sticky left-0 z-10 flex items-center gap-2 border-r border-b bg-background px-3 transition-colors",
            onEmployeeClick && "cursor-pointer hover:bg-muted/50",
            isSelected && "bg-primary/5"
          )}
          onClick={onEmployeeClick ? () => onEmployeeClick(employee) : undefined}
        >
          <Avatar size="sm">
