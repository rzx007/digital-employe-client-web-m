...
check "core zip exists (新命名最高版)" "BobanStaff-Core-0.1.16.zip" "$(basename "$CORE" 2>/dev/null)"
...
check "core 含最新deb(BobanStaff命名)" "yes" "$(echo "$corelist" | grep -q 'BobanStaff-Linux-arm64-0.1.16.deb$' && echo yes || echo no)"
check "core 不含旧版 0.1.3"  "no"  "$(echo "$corelist" | grep -q '0.1.3.deb$' && echo yes || echo no)"
...
check "core 不含旧版 0.1.1" "no"  "$(echo "$corelist" | grep -q '0.1.1.deb$' && echo yes || echo no)"
...
check "core SHA256SUMS"     "yes" "$([[ -f "$OUT/BobanStaff-Core-0.1.16.zip.sha256" ]] && echo yes || echo no)"
