...
2. ~~**A（技能在使用中自我改进）**~~ → ✅ **已落地**（spec [skill-self-improvement-and-lifecycle-curator-design.md](specs/2026-06-20-skill-self-improvement-and-lifecycle-curator-design.md) §4，计划 [skill-self-improvement-A.md](plans/2026-06-20-skill-self-improvement-A.md)）：员工 agent 新增 `update_skill` 工具，干活中发现已加载技能错/缺/过时即就地改。低分反馈的改进线索从「死文件」收编为持久 `skill_hints/`、加载时注入提示引导 update_skill、改后自动清除。经 spec/plan 双评审 + 逐任务两阶段审查 + 整体审查，后端 916 passed。
   > ⚠️ **2026-06-25 修订（路2，[技能单一真相 spec](specs/2026-06-25-skill-single-source-of-truth-design.md)）**：原 A 实现按 Hermes「广播」做法走 `fork-on-edit 固化到工作区库 → update_local_skill 写库 → sync_local_skill_to_assignees 全员同步`，即改进**集体共享**。现**刻意偏离 Hermes**，改为**只改调用者自己的私有副本**（不写库、不广播），改进**按员工隔离**——一个员工的修订不影响同事。配套把员工私有 `skills/` 目录确立为唯一真相、`EmployeeSkill` DB 降为派生投影（`reconcile_employee_skills`）。库的集中维护广播仍在（skill_api UI），但跳过已私改的员工。返工信号自动关联到具体技能仍属 v1.1。
3. ~~**B（技能/员工生命周期 curator）**~~ → ✅ **已落地**（spec 同上 §5，计划 [skill-lifecycle-curator-B.md](plans/2026-06-20-skill-lifecycle-curator-B.md)）：新增 `learning/curator.py`，搭 librarian 后台 pass（`run_librarian` 第 4 步）。技能按 last_used（分配时间为基线 + TaskExecutionLog/SkillRating 取 max）老化 active→stale(30d)→archived(90d)，**绝不删、pinned 豁免、可一键恢复**（restored_at 作新基线防即刻重归档）；archived 从 `available_skills` 逻辑隐藏。状态存 `<brain>/skill_lifecycle.json`（零迁移）。近重复技能候选合并（token 集相等 OR difflib≥0.85，只动候选）。员工闲置 90 天产**归档建议**（只读、绝不自动归档）。restore/pin 端点 + 成长面板 archived 折叠/恢复/置顶/归档建议。经 spec/plan 双评审 + 逐任务两阶段审查 + 整体审查，后端 954 passed、前端 tsc 净。
