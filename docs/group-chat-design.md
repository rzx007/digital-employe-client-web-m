...
sequenceDiagram
  ...
  G->>O: on_task_finalized → 更新群主可见上下文
  O->>G: 可选：全部完成后总汇报
```
...
| **G4** | 群时间线 `ExecutionReportCard` + 群 ArtifactPanel + 群主可选汇总 | **成员回执 + 文件感知** |
| **G5** | `depends_on` 门控、manifest、跨员工产物 prompt 注入              | 串行协作            |


G2～G4 形成可演示闭环；全程**无成员自由讨论**。
