...
- 回写授权码：**用评论**（真机验证：已通过的单**不能**改表单字段——飞书原生审批无
  「修正已通过实例表单」接口；只有 tasks/approve 能带 form，但已通过的单无待办任务）。
  - 加评论：`POST /open-apis/approval/v4/instances/{instance_code}/comments?user_id_type=user_id&user_id=<发起人>`
    body `{"content": "{\"text\":\"【激活授权码】<license>\"}"}` → 返回 `comment_id`（已验证 code 0）。
  - 读评论：`GET .../instances/{instance_code}/comments?...` → comments[].comment 是
    JSON 串 `{"text":"..."}`（已验证）。
- 权限：应用需「查看原生审批实例」+「评论审批实例」（实测加/读评论可用）。

> **2026-06-17 真机验证记录**（应用 cli_a9d1e24a... / 审批 BEBCCB45-...）：
> 换 token ✓ · 列实例(需 start_time/end_time 毫秒,窗口≤约) ✓ · 取实例表单值 ✓
> （设备码 input 直接取 value；截至日期 date 值为 `2026-06-17T00:00:00+08:00`，取前 10 位作 YYYY-MM-DD；
> **空字段不返回该 widget**）· 对已通过单加评论 ✓ · 读评论 ✓。
> **字段回写不可行→改评论**，这是真机逼出的修正。
