...
- 可用条件：聊天状态非 streaming/submitted。**既有会话视图、草稿视图、总管（curator）视图均支持**——草稿视图首条消息发送时 `doSend` 先创建会话再上传音频（见数据流）；总管视图有独立的 composer 挂载与发送路径（`curator-view.tsx`），需单独接线（2026-06-11 验收时发现遗漏后补）。**群聊不显示麦克风按钮**（既有群会话视图，以及草稿视图中目标联系人为群组 `contact.type === "group"` 时；首期仅单聊，见 YAGNI）。
