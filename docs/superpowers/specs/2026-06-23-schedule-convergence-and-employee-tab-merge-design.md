...
- 工作空间解析：复用 `ensure_user_default_workspace` 或按 user_id 取该用户工作空间集合下的 plans（与现有 user 维度一致；plan 通过 workspace_id 关联 user）。
- `employee_id` 参数：保留签名兼容但**忽略**（编排计划非员工维度；月历去员工 tab 后只剩工作台 workspace 级调用，不传 employee_id）。
...
**新增/改测试** `tests/test_monthly_calendar_orchestration.py`：recurring cron 月内展开正确（如每日/每周一）、once run_at 落当天、已跑 once 不再出现、跨用户/工作空间隔离、空结果。
...
- **保留并 retarget** `schedule-monitor/sections/schedule-calendar.tsx`：改读新 `runs` 契约——按天 `runs.length` 算密度色阶；hover/popover 列出当天计划运行（标题 + 时间 + recurring/once 标）。改 `use-schedule-monitor-queries.ts` 的 `useMonthlyScheduleOverview` 返回类型 + `types/schedule-monitor.ts` 的月历类型为 `runs`。
...
- 清理 orphan：`useTaskSummary`（仅 StatsCards 用，grep 确认零引用后删 + 其后端 `/tasks/schedule` summary 若无其它消费者标注给后端任务）；`TaskStatsCards` 组件文件删。`ScheduleCalendar`/`useMonthlyScheduleOverview`/`useExecutionMetrics7d`/`useTodayTaskRuns` 全保留（工作台/合并 tab 仍用）。
